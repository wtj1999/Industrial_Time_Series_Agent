...
class AnomalyTransformer(BaseDetector):
    ...
    def _train(self, windows, device):
        ...
        logger.info(
            "AnomalyTransformer train: %d epochs × %d batches "
            "(batch_size=%d, n_windows=%d, window_size=%d, "
            "d_model=%d, n_heads=%d, n_layers=%d) on %s",
            self.epochs, n_batches, self.batch_size,
            int(windows.shape[0]), self.window_size,
        ...
        for epoch in range(self.epochs):
            ...
            for (batch,) in loader:
                ...
                last_loss_min = float(loss_min.item())
            ...
            logger.info(
                ...
                "loss_min=%.6f (recon=%.6f, lambda*assdis=%.6f), "
                "loss_max=%.6f",
                epoch + 1, self.epochs,
