...
def save(model: Any, path: Any, metadata: dict | None = None) -> None:
    ...
    try:
        ...
        # Defensive fallback: model contains unpicklable members (e.g.,
        # PyOD's ts_lstm defines _Net as a local class inside __init__,
        # which pickle cannot serialize by qualified name). Fall back to
        # a metadata-only envelope so the artifact stays loadable; the
        # model field is None and a flag warns loaders.
        warnings.warn(
            "save(): model %s could not be pickled (%s). Falling back "
            "to a metadata-only envelope; the model itself cannot be "
            "reloaded. Re-fit on demand."
            % (type(model).__name__, exc),
            UserWarning, stacklevel=2,
        )
        stub = dict(envelope)
        stub["model"] = None
        stub["_model_unpicklable"] = True
        stub["_pickling_error"] = str(exc)
...
def _handle_loaded_object(
    ...
    if _is_envelope(obj):
        ...
        if obj.get("_model_unpicklable"):
            # Artifact was saved as metadata-only because the original
            # model could not be pickled. model field is None. Warn at
            # load time so callers don't silently get a None model.
            warnings.warn(
                "load(): artifact was saved as metadata-only because "
                "the original model could not be pickled "
                "(%s). model field is None; re-fit on demand."
                % obj.get("_pickling_error", "<unknown>"),
