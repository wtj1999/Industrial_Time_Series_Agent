"""Structured adapter for the battery-installation forecast application."""
...
class BatteryForecastParams(BaseModel):
    model: str = "sundial"
    granularity: str = "月度"
...
def build_query(raw_params: Dict[str, Any]) -> str:
    params = BatteryForecastParams.model_validate(raw_params)
    period_unit = "月" if params.granularity == "月度" else "季度"
    battery_types = _join_or_default(params.battery_types, "全部电池类型")
    ...
    return "\n".join([
        "请执行动力电池装车量时间序列预测任务。",
        f"预测目标：动力电池装车量；指定预测模型：{params.model}；时间粒度：{params.granularity}。",
        f"预测未来 {params.horizon} 个{period_unit}。",
        f"电池类型：{battery_types}。",
