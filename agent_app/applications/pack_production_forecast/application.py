"""Structured adapter for lithium-battery PACK production forecasting."""
...
class PackProductionForecastParams(BaseModel):
    model: str = "sundial"
    granularity: str = "月度"
    horizon: int = Field(default=6, ge=1, le=90)
    pack_segments: List[str] = Field(default_factory=list)
...
def build_query(raw_params: Dict[str, Any]) -> str:
    params = PackProductionForecastParams.model_validate(raw_params)
    period_unit = {"日度": "天", "周度": "周", "月度": "月"}[params.granularity]
    segments = _join_or_default(params.pack_segments, "全部PACK产品")
    ...
    return "\n".join([
        "请执行锂电PACK产量时间序列预测任务。",
        f"预测目标：锂电PACK产量；指定预测模型：{params.model}；时间粒度：{params.granularity}。",
