...
import { Database, Disc3, FileUp, Loader2, Play } from 'lucide-react';
...
import {
  ...
  DEFAULT_SERVO_MOTOR_REQUIREMENTS,
  DETECTION_MODELS,
  SERVO_MOTOR_ANOMALY_AGENT,
  SERVO_MOTOR_ANOMALY_TARGETS,
  buildServoMotorAnomalyQuery,
  type DetectionModel,
  type ServoMotorAnomalyTask,
  usesServoMotorWindow,
} from './config';

const MAX_FILE_MB = 100;
const fieldClass = 'h-10 w-full rounded-xl border border-steel-200 bg-white px-3 text-xs text-steel-700 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100';

export function ServoMotorAnomalyDetectionApp() {
  ...
  const [task, setTask] = useState<ServoMotorAnomalyTask>({
    anomalyTargets: ['驱动电流'],
    model: '自动推荐',
    contamination: 0.01,
    windowSize: 30,
    returnTopN: 20,
    randomState: 42,
    additionalRequirements: DEFAULT_SERVO_MOTOR_REQUIREMENTS,
  });
  ...
  const handleUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.split('.').pop()?.toLowerCase() !== 'csv') {
      setFormError('伺服电机异常检测当前仅支持 CSV 文件');
      return;
    }
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (usesServoMotorWindow(task) && (!Number.isInteger(task.windowSize) || task.windowSize < 2 || task.windowSize > 5000)) {
    ...
    void sendAgentTask(
      ...
      buildServoMotorAnomalyQuery(task),
      upload,
  ...
  if (started || items.length > 0) {
    return <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-steel-200/70 bg-sky-50/50 px-4 py-2.5 sm:px-6"><div className="mx-auto flex max-w-3xl items-center gap-2 text-xs text-steel-600"><Disc3 className="h-4 w-4 text-sky-700" /><span className="font-medium text-steel-800">{SERVO_MOTOR_ANOMALY_AGENT.name}</span><span className="text-steel-300">·</span><span>结构化任务已提交，可继续追问或补充要求</span></div></div>
  ...
  return <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6"><div className="mx-auto w-full max-w-4xl pb-8">
    <div className="mb-5 rounded-2xl border border-sky-200/80 bg-gradient-to-br from-sky-50 to-white p-5"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-sky-700"><Disc3 className="h-5 w-5" /></span><div><h2 className="text-base font-semibold text-steel-900">配置伺服电机异常检测任务</h2><p className="mt-1 text-xs leading-5 text-steel-500">从电气、运动与机械状态特征出发，识别负载异常、跟随偏差和性能退化。</p></div></div></div>
    <form onSubmit={handleSubmit} className="space-y-4">
      <FormSection number="1" title="选择检测数据" description="选择包含时间列及伺服电机关键状态特征的 CSV 文件。">
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]"><select value={selectedFileName} onChange={(event) => { setSelectedFileName(event.target.value); if (event.target.value) setUpload(null); }} disabled={datasetsLoading} className={fieldClass}><option value="">{datasetsLoading ? '正在加载我的数据…' : '从我的数据中选择'}</option>{datasets.filter((dataset) => dataset.extension === 'csv').map((dataset) => <option key={dataset.file_name} value={dataset.file_name}>{dataset.name}</option>)}</select><label className="inline-flex h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border border-steel-200 bg-white px-4 text-xs font-medium text-steel-700 transition-colors hover:border-sky-400 hover:bg-sky-50"><FileUp className="h-4 w-4" />上传新数据<input type="file" accept=".csv" onChange={handleUpload} className="hidden" /></label></div>
        {(upload || selectedDataset) && <div className="mt-3 flex items-center gap-2 rounded-lg bg-steel-50 px-3 py-2 text-xs text-steel-600"><Database className="h-3.5 w-3.5 text-sky-700" /><span className="font-medium">{upload?.name ?? selectedDataset?.name}</span></div>}
        {datasetError && <p className="mt-2 text-xs text-rose-600">{datasetError}</p>}
      </FormSection>

      <FormSection number="2" title="选择异常检测目标" description="框选数据中需要重点监测的伺服电机关键特征，可同时选择多个目标。">
        <div className="flex flex-wrap gap-2">{SERVO_MOTOR_ANOMALY_TARGETS.map((target) => <button key={target} type="button" onClick={() => toggleTarget(target)} className={cn('rounded-xl border px-3 py-2 text-xs transition-all', task.anomalyTargets.includes(target) ? 'border-sky-400 bg-sky-50 font-medium text-sky-800 shadow-sm' : 'border-steel-200 bg-white text-steel-600 hover:border-sky-300 hover:bg-sky-50/40')}>{target}</button>)}</div>
      ...
      <FormSection number="3" title="设置检测模型与参数" description="单目标自动推荐突变检测模型，多目标自动推荐多变量异常检测模型。">
        <div className={cn('grid gap-3', usesServoMotorWindow(task) ? 'sm:grid-cols-4' : 'sm:grid-cols-3')}>
          ...
          {usesServoMotorWindow(task) && <Field label="检测窗口（采样点）"><input type="number" min={2} max={5000} value={task.windowSize} onChange={(event) => setTask({ ...task, windowSize: Number(event.target.value) })} className={fieldClass} /></Field>}
      ...
      <FormSection number="4" title="补充要求（可选）" description="可指定关注时段、动作阶段、负载工况、报警事件或异常解释口径。"><textarea value={task.additionalRequirements} onChange={(event) => setTask({ ...task, additionalRequirements: event.target.value })} rows={4} maxLength={500} className="w-full resize-y rounded-xl border border-steel-200 bg-white px-3 py-2.5 text-xs leading-5 text-steel-700 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100" /></FormSection>
