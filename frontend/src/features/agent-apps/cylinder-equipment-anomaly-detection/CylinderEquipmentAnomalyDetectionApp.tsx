...
import { Database, FileUp, Gauge, Loader2, Play } from 'lucide-react';
...
import {
  ...
  CYLINDER_ANOMALY_TARGETS,
  CYLINDER_EQUIPMENT_ANOMALY_AGENT,
  DEFAULT_CYLINDER_ANOMALY_REQUIREMENTS,
  DETECTION_MODELS,
  buildCylinderAnomalyQuery,
  type CylinderAnomalyTask,
  type DetectionModel,
  usesCylinderDetectionWindow,
} from './config';

const MAX_FILE_MB = 100;
const fieldClass = 'h-10 w-full rounded-xl border border-steel-200 bg-white px-3 text-xs text-steel-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100';

export function CylinderEquipmentAnomalyDetectionApp() {
  ...
  const [task, setTask] = useState<CylinderAnomalyTask>({
    anomalyTargets: ['气缸压力'],
    model: '自动推荐',
    contamination: 0.01,
    windowSize: 30,
    returnTopN: 20,
    randomState: 42,
    additionalRequirements: DEFAULT_CYLINDER_ANOMALY_REQUIREMENTS,
  });
  ...
  const handleUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.split('.').pop()?.toLowerCase() !== 'csv') {
      setFormError('气缸设备异常检测当前仅支持 CSV 文件');
      return;
    }
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (usesCylinderDetectionWindow(task) && (!Number.isInteger(task.windowSize) || task.windowSize < 2 || task.windowSize > 5000)) {
    ...
    setStarted(true);
    void sendAgentTask(
      CYLINDER_EQUIPMENT_ANOMALY_AGENT.id,
      {
      ...
      buildCylinderAnomalyQuery(task),
      upload,
  ...
  if (started || items.length > 0) {
    return <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-steel-200/70 bg-indigo-50/50 px-4 py-2.5 sm:px-6"><div className="mx-auto flex max-w-3xl items-center gap-2 text-xs text-steel-600"><Gauge className="h-4 w-4 text-indigo-700" /><span className="font-medium text-steel-800">{CYLINDER_EQUIPMENT_ANOMALY_AGENT.name}</span><span className="text-steel-300">·</span><span>结构化任务已提交，可继续追问或补充要求</span></div></div>
  ...
  return <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6"><div className="mx-auto w-full max-w-4xl pb-8">
    <div className="mb-5 rounded-2xl border border-indigo-200/80 bg-gradient-to-br from-indigo-50 to-white p-5"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700"><Gauge className="h-5 w-5" /></span><div><h2 className="text-base font-semibold text-steel-900">配置气缸设备异常检测任务</h2><p className="mt-1 text-xs leading-5 text-steel-500">从气缸关键状态特征出发，识别突变、持续偏移及多特征协同异常。</p></div></div></div>
    <form onSubmit={handleSubmit} className="space-y-4">
      <FormSection number="1" title="选择检测数据" description="选择包含时间列及气缸关键状态特征的 CSV 文件。">
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]"><select value={selectedFileName} onChange={(event) => { setSelectedFileName(event.target.value); if (event.target.value) setUpload(null); }} disabled={datasetsLoading} className={fieldClass}><option value="">{datasetsLoading ? '正在加载我的数据…' : '从我的数据中选择'}</option>{datasets.filter((dataset) => dataset.extension === 'csv').map((dataset) => <option key={dataset.file_name} value={dataset.file_name}>{dataset.name}</option>)}</select><label className="inline-flex h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border border-steel-200 bg-white px-4 text-xs font-medium text-steel-700 transition-colors hover:border-indigo-400 hover:bg-indigo-50"><FileUp className="h-4 w-4" />上传新数据<input type="file" accept=".csv" onChange={handleUpload} className="hidden" /></label></div>
        {(upload || selectedDataset) && <div className="mt-3 flex items-center gap-2 rounded-lg bg-steel-50 px-3 py-2 text-xs text-steel-600"><Database className="h-3.5 w-3.5 text-indigo-700" /><span className="font-medium">{upload?.name ?? selectedDataset?.name}</span></div>}
        {datasetError && <p className="mt-2 text-xs text-rose-600">{datasetError}</p>}
      </FormSection>

      <FormSection number="2" title="选择异常检测目标" description="框选数据中需要重点监测的气缸关键特征，可同时选择多个目标。">
        <div className="flex flex-wrap gap-2">{CYLINDER_ANOMALY_TARGETS.map((target) => <button key={target} type="button" onClick={() => toggleTarget(target)} className={cn('rounded-xl border px-3 py-2 text-xs transition-all', task.anomalyTargets.includes(target) ? 'border-indigo-400 bg-indigo-50 font-medium text-indigo-800 shadow-sm' : 'border-steel-200 bg-white text-steel-600 hover:border-indigo-300 hover:bg-indigo-50/40')}>{target}</button>)}</div>
      ...
      <FormSection number="3" title="设置检测模型与参数" description="单目标自动推荐突变检测模型，多目标自动推荐多变量异常检测模型。">
        <div className={cn('grid gap-3', usesCylinderDetectionWindow(task) ? 'sm:grid-cols-4' : 'sm:grid-cols-3')}>
          ...
          {usesCylinderDetectionWindow(task) && <Field label="检测窗口（采样点）"><input type="number" min={2} max={5000} value={task.windowSize} onChange={(event) => setTask({ ...task, windowSize: Number(event.target.value) })} className={fieldClass} /></Field>}
      ...
      <FormSection number="4" title="补充要求（可选）" description="可指定关注时段、设备动作阶段、故障类型或异常解释口径。"><textarea value={task.additionalRequirements} onChange={(event) => setTask({ ...task, additionalRequirements: event.target.value })} rows={4} maxLength={500} className="w-full resize-y rounded-xl border border-steel-200 bg-white px-3 py-2.5 text-xs leading-5 text-steel-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" /></FormSection>
